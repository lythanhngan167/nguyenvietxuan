...
class Request_packageModelRequestpackages extends \Joomla\CMS\MVC\Model\ListModel
{
	...
	protected function getListQuery()
	{
		...
		if (!Factory::getUser()->authorise('core.edit', 'com_request_package'))
		{
		...
		$user = JFactory::getUser();
		if($user->id > 0) {
			$query->where("created_by.id =" . $db->quote($user->id));
		}
	...
	public function getItems()
	{
		...
		foreach ($items as $item)
		{
			...
			$item->province = JText::_('COM_REGISTRATION_PROVINCE_' . strtoupper($item->province));

			$item->status = JText::_('COM_REQUEST_PACKAGE_REQUESTPACKAGES_STATUS_OPTION_' . strtoupper($item->status));

			// Get the title of every option selected
			$options      = explode(',', $item->company);
			$options_text = array();
			...
			$item->company = !empty($options_text) ? implode(',', $options_text) : $item->company;

			$item->services = JText::_('COM_REQUEST_PACKAGE_REQUESTPACKAGES_SERVICES_OPTION_' . strtoupper($item->services));
		}
	...
	protected function loadFormData()
	{
		...
		if ($error_dateformat)
		{
			$app->enqueueMessage(Text::_("COM_REQUEST_PACKAGE_SEARCH_FILTER_DATE_FORMAT"), "warning");
