...
class AffiliateHelper{
					...
					if ($params->get('multi-level', '1')) {
						...
						if (!empty($data['component']))
							if (!empty($account->parent_id) && $account->parent_id != $account->id)
								self::createParentConversions($data, $account->parent_id, 2);
					}
	...
	/**
	 * This function does nothing on the normal version
	 */
	private static function createParentConversions($data, $parent_id, $level) {
	...
	static function nav_tabs(){
		...
		$return = "<ul class='nav nav-tabs com_affiliatetracker'>
