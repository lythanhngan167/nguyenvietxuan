 ...
 * @since  1.6
 */
class ReferralModelReferral extends \Joomla\CMS\MVC\Model\ItemModel
{
    public $_item;
	...
	{
		$app  = Factory::getApplication('com_referral');
		$user = Factory::getUser();

		// Check published state
		if ((!$user->authorise('core.edit.state', 'com_referral')) && (!$user->authorise('core.edit', 'com_referral')))
		{
	...
	public function getItem($id = null)
	{
        if ($this->_item === null)
        {
            ...
            if ($table && $table->load($id))
            {
                ...
                if ($published = $this->getState('filter.published'))
                {
                    ...
                    {
                        throw new Exception(Text::_('COM_REFERRAL_ITEM_NOT_LOADED'), 403);
                    }
                }
            ...
            if (empty($this->_item))
			{
				throw new Exception(Text::_('COM_REFERRAL_ITEM_NOT_LOADED'), 404);
			}
		}
		...
		{
			if (is_object($this->_item->cat_id))
			{
				$this->_item->cat_id = ArrayHelper::fromObject($this->_item->cat_id);
			}

			if (is_array($this->_item->cat_id))
			{
				$this->_item->cat_id = implode(',', $this->_item->cat_id);
			}
			...
			$query
				...
				->where('FIND_IN_SET(' . $db->quoteName('id') . ', ' . $db->quote($this->_item->cat_id) . ')');

			$db->setQuery($query);

			$result = $db->loadColumn();

			$this->_item->cat_id = !empty($result) ? implode(', ', $result) : '';
		}
	...
	public function getItemIdByAlias($alias)
	{
            ...
            {
            	$aliasKey   = $this->getAliasFieldNameByView('referral');
            }
