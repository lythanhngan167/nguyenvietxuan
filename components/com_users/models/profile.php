...
class UsersModelProfile extends JModelForm
{
	...
	public function createVerifyCode($params = array())
    {
      ...
      $obj->social_type = $params['socialType'];
			...
			$obj->id = $user->id;
      if ($db->insertObject('#__social', $obj)) {
	...
	public function doVerifyPhone($code, $tokenCode, $username) {
		$db = JFactory::getDbo();
		$sql = 'SELECT COUNT(*) FROM #__social WHERE `token` = ' . $db->quote($tokenCode)
                    . ' AND verify_code = ' . $db->quote($code)
                    . ' AND field_value = ' . $db->quote($username);
	...
	//user get verify time
	public function getUserVerifyTime($phone) {
		$db = JFactory::getDbo();
		$user = JFactory::getUser();
		date_default_timezone_set('Asia/Ho_Chi_Minh');
		$last24Hour = date("Y-m-d H:i:s",strtotime("-24 hours"));

		$sql = 'SELECT COUNT(*)
				FROM #__social
                WHERE field_value = ' . $db->quote($phone) .
				' AND created_date >= ' . $db->quote($last24Hour);
