...
class UsersControllerUser extends UsersController
{
	...
	public function login()
	{
		...
		$sql = 'SELECT username
				FROM #__users
				WHERE username=' . $db->quote($data['username']) . 'OR username2=' . $db->quote($data['username']) . 'OR id_biznet=' . $db->quote($data['username']);
		...
		if (empty($data['return']))
		{
			...
			$data['return'] = 'index.php?Itemid=276';
		}
		...
		{
				if ($group == 2) {
					$data['return'] = 'index.php?Itemid=348';
					$app->redirect(JRoute::_('index.php?Itemid=348', false));
					break;
				}else{
					if ($group == 3) {
						$data['return'] = 'index.php?Itemid=276';
						$app->redirect(JRoute::_('index.php?Itemid=276', false));
						break;
					}
