...
class K2ViewItem extends K2View
{
    public function display($tpl = null)
    {
        ...
        if (
            ...
            if($_REQUEST['preview'] == 1){ // preview mode
        ...
        if ($item->params->get('itemAuthorLatest') && $item->created_by_alias == '') {
            ...
            $item->params->set('itemAuthorLatestLimit',6);
            //$authorLatestItems = $model->getAuthorLatest($item->id, $item->params->get('itemAuthorLatestLimit'), $item->created_by);
            $itemsByCat = $model->getItemByCategory($item->id, $item->params->get('itemAuthorLatestLimit'), $item->catid);
            ...
            // }
            if (count($itemsByCat)) {
                for ($i = 0; $i < count($itemsByCat); $i++) {
                    $itemsByCat[$i]->link = urldecode(JRoute::_(K2HelperRoute::getItemRoute($itemsByCat[$i]->id.':'.urlencode($itemsByCat[$i]->alias), $itemsByCat[$i]->catid.':'.urlencode($itemsByCat[$i]->categoryalias))));
                }
                $this->assignRef('itemsByCat', $itemsByCat);
            }
