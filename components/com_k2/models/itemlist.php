...
class K2ModelItemlist extends K2Model
{
    ...
    public function getData($ordering = null)
    {
        ...
        */
        ...
        if($ordering == 'topItem'){
            $query .= " AND i.top_item = 1 AND i.top_item_date = (SELECT MAX(top_item_date) FROM #__k2_items)";
        }
        // Set ordering
        switch ($ordering) {
            case 'date':
    ...
    public function getItemByCategory($itemID, $limit, $catID)
    {
        $app = JFactory::getApplication();
        $user = JFactory::getUser();
        $aid = (int)$user->get('aid');
