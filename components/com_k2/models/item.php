...
class K2ModelItem extends K2Model
{
    ...
    public function comment()
    {
        ...
        if ((($params->get('comments') == '2') && ($user->id > 0) && K2HelperPermissions::canAddComment($item->catid)) || ($params->get('comments') == '1')) {
            ...
            if (empty($userName) || $userName == JText::_('K2_ENTER_YOUR_NAME') || empty($commentText) || $commentText == JText::_('K2_ENTER_YOUR_MESSAGE_HERE')) {
                $response->message = JText::_('K2_YOU_NEED_TO_FILL_IN_ALL_REQUIRED_FIELDS');
                $response->cssClass = 'k2FormLogError';
                echo json_encode($response);
                $app->close();
            }
            // if (!JMailHelper::isEmailAddress($commentEmail) && !empty($commentEmail) && $commentEmail != JText::_('K2_ENTER_YOUR_EMAIL_ADDRESS')) {
            ...
            //     $response->message = JText::_('K2_INVALID_PHONE');
            //     $response->cssClass = 'k2FormLogError';
            //     echo json_encode($response);
            //     $app->close();
            // }
    ...
    protected function isPhone() {
		...
		return preg_match('/^0[0-9]{9}+$/', $results);
	}
}
