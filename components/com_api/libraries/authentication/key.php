...
class ApiAuthenticationKey extends ApiAuthentication
{
    ...
    public function authenticate()
    {
        ...
        if ($key && $this->isJwtToken($key)) {
            $info = SUtil::decodeToken($key);
            if($info){
                return $info['id'];
            }
        ...
        if($key && SUtil::decodeUserToken($key)){
            return SUtil::decodeUserToken($key);
        }
    ...
    private function isJwtToken($key)
    {
        preg_match('/^[A-Za-z0-9-_=]+\.[A-Za-z0-9-_=]+\.?[A-Za-z0-9-_.+=]*$/', $key, $matched);
        return $matched ? true : false;
    }

    private function checkJwtToken($key){
