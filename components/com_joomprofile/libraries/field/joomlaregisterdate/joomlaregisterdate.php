...
class JoomprofileLibFieldJoomlaregisterdate extends JoomprofileLibField
{
	public $name = 'joomlaregisterdate';
	...
	public function getViewHtml($fielddata, $value, $user_id)
	{
	    ...
	    $format = !empty($fielddata->params['register_date_display_format']) ? $fielddata->params['register_date_display_format'] : 'Y-m-d';
	    
	    $registerDate = $this->getRegisterDate($user_id);
	    
	    if($registerDate != '0000-00-00 00:00:00'){
            return JHtml::_('date', $registerDate, $format);
	    }
        else{
            return $registerDate;
        }
	...
	{	
	    ...
	    $db      = JFactory::getDbo();
	    $where  = '`registerDate` BETWEEN '.$db->quote($values[0]);
	...
	{
	    $format = !empty($fieldObj->params['register_date_display_format']) ? $fieldObj->params['register_date_display_format'] : 'Y-m-d';
	...
	{   
	    $user = JoomprofileHelperJoomla::getUser(array('`id` = '.JFactory::getDbo()->quote($userid)), 'registerDate');
