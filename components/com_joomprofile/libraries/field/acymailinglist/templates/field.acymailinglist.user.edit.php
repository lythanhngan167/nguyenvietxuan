...
<?php if($fielddata->params['multi_acymailing_list']) :?>
	...
	<?php foreach ($data->allowed_list as $list) : ?>
		...
		<label class="checkbox stacked">
			...
			<?php echo (is_array($user_list) && in_array($list->listid, $user_list)) ? 'checked="checked"' : '';?>/>
			<?php echo $list->name;?>
		</label>
	<?php endforeach;?>
...
<?php else:?>
	<div class="controls">						
        ...
        <?php echo ($fielddata->mapping->required  == true) ? ' required="required" data-validation-required-message="'.JText::_('COM_JOOMPROFILE_VALIDATION_REQUIRED').'"' : ''; ?>>
    		...
    		<?php foreach ($data->allowed_list as $list) : ?>
    			<option value="<?php echo $list->listid?>" <?php echo (is_array($user_list) && in_array($list->listid, $user_list)) ? 'selected="selected"' : '';?>><?php echo $list->name;?></option>
    		<?php endforeach;?>
		</select>
	</div>
<?php endif;?> 
