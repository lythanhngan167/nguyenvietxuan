...
defined('_JEXEC') or die('Restricted access'); 
 ...
 if(!empty($this->payment->payment_method_id)){$payment_method_id = $this->payment->payment_method_id;}else {$payment_method_id="";}
        ...
        //Header Information
			  ...
			  $bill_info .='<td style="border: 0px;" width="50%" class="payment-to" > <b>'.JText::_('LABEL_PAYMENT_INVOICE_PAYMENT_TO').'</b> <br />';
				...
				$payment_type_fee = $model->getPayammount($item,'fee');
					$invoice_table .= '<tr>'; 
								$invoice_table .= '<td align="right" style="width: 25%;padding: 30px;text-align: right; ">'.number_format($payment_type_fee, 2).'</td>';
								$invoice_table .= '<td style="padding: 30px; text-align: left;"  >'.$payment_type_name.'</td>';
								$invoice_table .= '<td align="center"  style="width: 10%;padding: 30px;;">'.$i.'</td>';
					$invoice_table .= '</tr>'; 
				}
				
					$invoice_table .= '<tr>'; 
								$invoice_table .= '<td align="right" style="width: 25%;padding:10px 30px; text-align: right;">'.number_format($total_bill, 2).'</td>';
								$invoice_table .= '<td align="right" style="padding:10px 30px; border-left: 0 none;text-align: left;" > <span> :'.JText::_('LABEL_PAYMENT_INVOICE_SUB_TOTAL').' </span></td>';
								$invoice_table .= '<td align="center"  style="width: 10%;padding:10px 30px;border-right: 0 none; "></td>';
								
					$invoice_table .= '</tr>'; 
					$invoice_table .= '<tr>'; 
								$invoice_table .= '<td align="right" style="width: 25%;padding:10px 30px; text-align: right;">'.number_format($paid_ammount, 2).'</td>';
								$invoice_table .= '<td align="right" style="padding:10px 30px;border-left: 0 none;text-align: left; " > <span> :'.JText::_('LABEL_PAYMENT_INVOICE_PAID').' </span></td>';
								$invoice_table .= '<td align="center"  style="width: 10%;padding:10px 30px;border-right: 0 none; "></td>';
								
					$invoice_table .= '</tr>'; 
					$invoice_table .= '<tr>'; 
								$invoice_table .= '<td align="right" style="width: 25%;padding:10px 30px; text-align: right;">'.$due.'</td>';
								$invoice_table .= '<td align="right" style="padding:10px 30px;border-left: 0 none;text-align: left; " > <span> :'.JText::_('LABEL_PAYMENT_INVOICE_DUE_AMMOUNT').' </span></td>';
								$invoice_table .= '<td align="center"  style="width: 10%;padding:10px 30px;border-right: 0 none; "></td>';
