...
class EShopControllerCheckout extends JControllerLegacy
{
    ...
    public function processOrder()
    {
        ...
        if ($cartAmount < $config->minCartAmount) {
            JFactory::getApplication()->redirect(JRoute::_(EshopRoute::getViewRoute('checkout') . '&layout=cart_min_amount'));
            exit();
        }
        ...
        if (!$return) {
            ...
            if ($cart->hasShipping()) {
                ...
                if (is_array($shippingMethod)) {
                    ...
                    $data['shipping_lng'] = $shippingMethod['lng'];
                    $data['shipping_lat'] = $shippingMethod['lat'];
                } else {
            ...
            $data['delivery_hour'] =  $input->getString('delivery_hour');
            //$session->get('delivery_hour');
    ...
    /**
     * Tung add
     * Function to process order on view
     */
    public function processOrderOneView()
    {
        JSession::checkToken('post') or jexit(JText::_('JInvalid_Token'));
        ...
        $user = JFactory::getUser();
        $filterInput = JFilterInput::getInstance(null, null, 1, 1);
        $dataForm = $this->input->post->getArray();
        $dataForm = $filterInput->clean($dataForm, null);
        /* @var EShopModelCheckout $model */
        $model = $this->getModel('Checkout');
        $cart = new EshopCart();
        ...
        if ($cart->hasShipping()) {
            // Validate if shipping address is set
            if ($user->get('id') && $dataForm['address_id']) {
                $shippingAddress = EshopHelper::getAddress($dataForm['address_id']);
            } else {
                $guest = $session->get('guest');
                $shippingAddress = isset($guest['shipping']) ? $guest['shipping'] : '';
            }
        ...
        // Validate if payment address has been set.
        if ($user->get('id') && $dataForm['shipping_address']) {
            $paymentAddress = EshopHelper::getAddress($dataForm['address_id']);
        } else {
            $guest = $session->get('guest');
            $paymentAddress = isset($guest['payment']) ? $guest['payment'] : '';
        }
        ...
        if (!$return) {
            ...
            $data['payment_zone_name'] = $paymentAddress['zone_name'];
            $data['payment_country_name'] = $paymentAddress['country_name'];
            $data['payment_method'] = $dataForm['payment_method'];
            $data['payment_method_title'] = EshopHelper::getPaymentTitle($data['payment_method']);
            ...
            if ($cart->hasShipping()) {
                ...
                $data['shipping_zone_name'] = $shippingAddress['zone_name'];
                $data['shipping_country_name'] = $shippingAddress['country_name'];
                $shippingMethod = $dataForm['shipping_method'];
            ...
            $data['totals'] = $totalData;
            $data['delivery_date'] = $session->get('delivery_date');
            $data['delivery_hour'] = $session->get('delivery_hour');
            $data['comment'] = $session->get('comment');
            $data['order_status_id'] = EshopHelper::getConfigValue('order_status_id');
            $data['language'] = JFactory::getLanguage()->getTag();
            $currency = new EshopCurrency();
    ...
    public function processOrderAPI()
    {
        ...
        if ($test == 1) {
            ...
            $products[] = array('product_id' => 5);
            $products[] = array('qty' => 1);
            $products[] = array('product_id' => 6);
            $products[] = array('qty' => 1);
        }
        ...
        if ($user->get('id')) {
            ...
            $data['firstname'] = $_POST['payment_firstname'];
            $data['lastname'] = $_POST['payment_lastname'];
            $data['email'] = $_POST['payment_email'];
            $data['telephone'] = $_POST['payment_telephone'];
            $data['fax'] = $_POST['payment_fax'];
        ...
        $data['shipping_country_name'] = $_POST['shipping_country_name'];
        $data['shipping_zone_id'] = $_POST['shipping_zone_id'];
        $data['shipping_zone_name'] = $_POST['shipping_zone_name'];
        ...
        $data['totals'] = $totalData;
        $data['delivery_date'] = JFactory::getDate()->toSql();
        ...
        if ($user_id > 0 || $user_id == 0) {
            ...
            if ($result) {
                $dataReSult['ok'] = 1;
                $dataReSult['order_number'] = $data['order_number'];
                $dataReSult['order_id'] = EshopHelper::getOrderID($data['order_number']);
    ...
    public function processOrderAPIOld()
    {
