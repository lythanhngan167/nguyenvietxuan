 ...
 if ( $i % 3 == 0 ) { echo '<div class="', $class_row, '">'; } ?>
    ...
    <ul>
        <?php foreach ( $this->data[ $key ] as $item ) :
          ...
          if (strpos($item->href, '/images/') !== false) {
              continue;
          }
          if (strpos($item->href, '/media/') !== false) {
              continue;
          }
          if (strpos($item->href, '/author/') !== false) {
              continue;
          }
          if (strpos($item->href, '?format=feed') !== false) {
              continue;
          }
          //echo $item->href;
          ?>
...
<p id="aimy-sitemap-credits">
    <a href="https://www.aimy-extensions.com/joomla/sitemap.html" target="_blank"><?php
