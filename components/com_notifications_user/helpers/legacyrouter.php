 ...
 * @since  1.6
 */
class Notifications_userRulesLegacy implements RulesInterface
{
    /**
    ...
    public function build(&$query, &$segments)
    {
        ...
        if (isset($query['id']))
        {
            ...
            if ($view !== null)
            {
				...
				if ($view == 'notificationuser')
				{
					$segments[] = $query['id'];
				}
				if ($view == 'notificationusers')
				{
					$category = Categories::getInstance('notifications_user.notificationusers')->get($query['id']);
    ...
    public function parse(&$segments, &$vars)
    {
        ...
        $model        = Notifications_userHelpersNotifications_user::getModel($vars['view']);
        ...
        while (!empty($segments))
        {
            ...
            else
            {
				...
				if ($vars['view'] == 'notificationusers')
				{
					$category = Categories::getInstance('notifications_user.notificationusers', array('access' => false))->get('root');
					if ($category)
					{
