...
class UsersApiResourceProjects extends ApiResource
{
    ...
    public function post()
    {
        ...
        if(@$data['include_project']){
            ...
            $level_agency = $user->get('level');
            if(isset($level_agency)){
                $config = new JConfig();
                $project_ids=$config->project_setup_level['level_'.$level_agency];
                if(count($project_ids)>0){
                    $params['where'][]='id IN (' . implode(',', ArrayHelper::toInteger($project_ids)) . ')';
                }
            }
        }
        $dao = new ProjectDao();
