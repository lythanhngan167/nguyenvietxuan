...
/// added K2ModelItemlistEasyFilterEasyFilter
class K2ModelItemlistEasyFilter extends K2Model {
							...
							case 'easyfilter':
								...
								if($keyword != "") {
									$query .= " AND (
												 ...
												 OR i.introtext LIKE '%{$keyword}%' 
												 OR i.fulltext LIKE '%{$keyword}%'
												 OR i.extra_fields_search LIKE '%{$keyword}%'
											)";
								}
							
								$tag = JRequest::getString('etag');
								...
								//category
								$catid = JRequest::getInt("ecategory", 0);
								if($catid != 0) {
									$query .= " AND ( ";
									$subs = K2ModelItemlistEasyFilter::getCategoryTree($catid);
								...
								if (JRequest::getVar('restcat') != '') {
									...
									foreach($restcat as $kr => $restcatid) {
										$restsubs = K2ModelItemlistEasyFilter::getCategoryTree($restcatid);
								...
								//author
								$author = JRequest::getInt("eauthor", 0);
								if($author != 0) {
									$query .= " AND i.created_by={$author}";
								}
								...
								if(!empty($created_from) && !empty($created_to)) {
									$query .= " AND i.created >= '{$created_from}' AND i.created <= '{$created_to} 23:59:59'";
								}
						...
						default:
								...
								if (is_array($searchIDs) && count($searchIDs)) {
										...
										if ($params->get('catCatalogMode')) {
												...
												$result = K2ModelItemlistEasyFilter::getCategoryTree($searchIDs);
												if (count($result)) {
														$sql = @implode(',', $result);
														$query .= " AND i.catid IN ({$sql})";
												}
		...
		function getTotal() {
				...
				switch ($task) {
							...
							case 'easyfilter':
								...
								$keyword = JRequest::getVar("keyword", "");
								if($keyword != "") {
									$query .= " AND (i.title LIKE '%{$keyword}%' OR i.introtext LIKE '%{$keyword}%' OR i.fulltext LIKE '%{$keyword}%')";
								}
		...
		function getCategoryChildren($catid, $clear = false) {
			...
			foreach ($rows as $row) {
				...
				if (K2ModelItemlistEasyFilter::hasChildren($row->id)) {
					K2ModelItemlistEasyFilter::getCategoryChildren($row->id);
				}
			}
			return $array;
		}
		...
		function countCategoryItems($id) {
				...
				$categories = K2ModelItemlistEasyFilter::getCategoryTree($id);
			...
			if (JFile::exists(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_joomfish'.DS.'joomfish.php') && $currentLang != $defaultLang) {
				...
				if ($type=='exact') {
					...
					if (JString::strlen($word) > 3 && !in_array($word, $search_ignore)) {
						...
						$jfQuery = " SELECT reference_id FROM #__jf_content as jfc LEFT JOIN #__languages as jfl ON jfc.language_id = jfl.lang_".K2_JF_ID;
						$jfQuery .= " WHERE jfc.reference_table = 'k2_items'";
						$jfQuery .= " AND jfl.lang_code=".$db->Quote($currentLang);
