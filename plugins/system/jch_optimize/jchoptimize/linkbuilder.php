...
class LinkBuilder
{
		...
		if (!Helper::isMsieLT10() && $this->params->get('combine_files_enable', '1') && !$this->oParser->bAmpPage)
		{
			...
			if ($bCombineCss && !empty($aLinks['css']))
			{
				foreach ($aLinks['css'] as $aCssLinks)
				{
					...
					##<procode>##
					//$this->http2PushBgImages($aCssCache);
					##</procode>##
				}
	...
	protected function replaceLinks($sId, $sType, $sSection = 'head', $bLastJsFile = false)
	{
		...
		if ($sType == 'js' && $bLastJsFile && !$this->oParser->bExclude_js)
		{
			...
			##<procode>##
			$deferred = $this->oParser->isFileDeferred($sNewLink);
...
CSSASYNC;
	...
	public function http2PushBgImages($aCssCache)
	{
		$oCssParser = new Cssparser($this->params);

		foreach ($aCssCache['file'] as $sCss)
		{
			try
			{
				$oCssParser->correctUrl($sCss, array(), false, true);
			}
			catch (Exception $e)
			{
			}
