	...
	public function renderSubmenu($layoutFile)
	{
		if (is_file($layoutFile))
		{
			...
			foreach ($children as $child)
			{
                ...
                //Fixbug get id of submenu
              $chid->id = $child->get('id');
	...
	protected function populateTree($levels)
	{
		foreach ($levels as $item)
		{
			...
			elseif ($item->type === 'heading')
			{
				...
				$this->tree->addChild(new Node\Heading($item->title, $class, $item->id, $item->icon), $this->enabled);
			...
			{
				$cNode = new Node\Url($item->title, $item->link, $item->browserNav, $class, $item->id, $item->icon);
			...
			{
				$cNode = new Node\Component($item->title, $item->element, $item->link, $item->browserNav, $class, $item->id, $item->icon);
				$this->tree->addChild($cNode, $this->enabled);
			...
			elseif ($item->type === 'container')
			{
				...
				$this->tree->addChild(new Node\Container($item->title, $item->class, $item->id, $item->icon), $this->enabled);
