 ...
 */
 defined( '_JEXEC' ) or die(); function _render_button( $route, $icon, $i18n, $dis = false, $title = false ) { $href = $dis ? '#' : JRoute::_( 'index.php?' . $route ); $cls = 'aimy-btn' . ( $dis ? ' aimy-btn-disabled' : '' ); $text = JText::_( $i18n ); $attr = $dis ? 'disabled="disabled" onclick="alert(this.title);return false;"' : ''; if ( $title === false ) { $title = $text; } else { $title = JText::_( $title ); } echo '<a href="', $href, '" class="', $cls, '" ', $attr, ' title="', htmlspecialchars( $title ), '">', '<span class="icon">', '<i class="aimy-icon-', $icon, ' aimy-icon-hg"></i>', '</span>', '<span class="text">', $text, '</span>', '</a>'; } ?>
