 ...
 * @since  1.6
 */
class OrderViewReports extends JViewLegacy
{
    protected $items;
    ...
    protected function addToolbar()
    {
		...
		$sale_id = $state->get('filter.sale_id');
		$sale = JFactory::getUser($sale_id);
		JToolBarHelper::title('Nhân viên '.$sale->name, 'orders.png');
        /*
        $state = $this->get('State');
	...
	public function getStatus($item){
		if($item->category_id != 150){
			return '';
		}
		return 'Trả lại';
	}

	public function getPayBackStatus($item){
		if($item->category_id != 150){
			return '';
		}
		if($item->payback == 1){
			return '<p class="text-success">Đã hoàn tiền</p>';
		}
		return '<p class="text-danger">Chưa hoàn tiền</p>';
	}

	function showCheckbox($item){
		if($item->category_id == 150 && $item->payback == 0){
            return true;
        }
        return false;
	}
