...
class K2ViewItem extends K2View
{
    public function display($tpl = null)
    {
        ...
        if($_REQUEST['action'] == 'sendNotificationsPost'){ // for testing
          $title = 'Cập nhật ứng dụng phiên bản 1.10';
          $description = 'Cập nhật ứng dụng phiên bản 1.10 để sử dụng những tính năng mới nhất.';
          $page_app = 'insurchannel://bizappco/NewsCommonPage/86';
          $web_url = 'https://bcavietnam.com/blog/430-bca-insurance-co-lua-dao-khong.html';
          $model->sendNotificationsPost($title,$description,$page_app,$web_url);
          echo $page_app;
          echo "<br>";
          echo $web_url;
          die;
        ...
        $lists['top_item_date'] = $top_item_date;
        ...
        $lists['top_item'] = JHTML::_('select.booleanlist', 'top_item', 'class="inputbox"', $item->top_item);
        $lists['send_notication'] = JHTML::_('select.booleanlist', 'send_notication', 'class="inputbox"', 0);
        ...
        if (isset($item->user_send_notification)) {
            $user_send_notification = JUser::getInstance($item->user_send_notification);
            $item->user_send_notification = $user_send_notification->name;
        }
        ...
        $lists['groups'] = $groups[0];
        if($lists['groups'] == 4){
            $a[] = JHTML::_('select.option', 0, JText::_('K2_SELECT_CATEGORY'));
            $categories = $categoriesModel->categoriesTree(null, true, false);
            $config = new JConfig();
            if(isset($config->showed_cat_k2)){
                foreach($config->showed_cat_k2 as $showed_cat_k2){
                    for ($i = 0; $i < count($categories); $i++) {
                        if($showed_cat_k2 == $categories[$i]->value){
                            array_push($a,(array) $categories[$i]);

                        }
                    }
                }
            }
        $lists['categories'] = JHTML::_('select.genericlist', $a, 'catid', '', 'value', 'text', $item->catid);
        ...
        */
        ...
        if(isset($config->categories_post_send_notification) && isset($item->catid)){
            $str_parens = implode(', ', $config->categories_post_send_notification);
            $db = JFactory::getDbo();
            ...
            if(in_array($item->catid,$result) || in_array( $item->catid,$config->categories_post_send_notification)){
