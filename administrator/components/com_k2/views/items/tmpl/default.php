            ...
            <table class="adminlist table table-striped<?php if (isset($this->rows) && count($this->rows) == 0): ?> nocontent<?php endif; ?>" id="k2ItemsList">
                <thead>
                    <tr>
                        ...
                        <th class="k2ui-center">
                            <?php echo JHTML::_('grid.sort', 'K2_PUBLISHED_BY', 'i.published_by', @$this->lists['order_Dir'], @$this->lists['order']); ?>
                        </th>
                        <th class="k2ui-center">
                            <?php echo JHTML::_('grid.sort', 'K2_TOP_ITEM', 'i.top_item', @$this->lists['order_Dir'], @$this->lists['order']); ?>
                        </th>
                        <?php if (K2_JVERSION != '30'): ?>
                        <th class="k2ui-order">
                            <?php if ($this->filter_featured == '1'): ?>
                ...
                <tbody>
                    ...
                    <tr class="row<?php echo $key%2; ?>"<?php echo ($this->filter_featured != '1') ? ' sortable-group-id="'.$row->catid.'"' : ''; ?>>
                        ...
                        <td>
                            <div class="k2ui-list-title">
                                ...
                                <?php if ($row->published == 0): ?>
                                  <?php
                                  $itemURL = urldecode(JRoute::_(K2HelperRoute::getItemRoute($row->id.':'.urlencode($row->alias),$row->catid.':'.$row->aliascat)));

                                  $previewLink =  str_replace("administrator/","",$itemURL);
                                   ?>
                                  <div class="k2ListItemDisabledPreview">
                                     <a class="k2ListItemDisabledPreview" href="<?php echo $previewLink; ?>&preview=1" target="_blank">
                                     <span class="label label-warning">Xem trước</span>
                                   </a>
                                 </div>
                                <?php endif; ?>
                                <?php if ($row->published == 1): ?>
                                  ...
                                  $previewLink =  str_replace("administrator/","",$itemURL);
                                   ?>
                                  <div class="k2ListItemDisabledPreview">
                                     <a class="k2ListItemDisabledPreview" href="<?php echo $previewLink; ?>" target="_blank">
                                     <span class="label label-success">Xem Bài viết</span>
                                   </a>
                                 </div>
                                <?php endif; ?>
                            </div>
                        ...
                        <td class="k2ui-center">
                          <?php if ($this->user->gid > 23 && $context != "modalselector"): ?>
                          <a href="<?php echo JRoute::_('index.php?option=com_k2&view=user&cid='.$row->published_by); ?>">
                            <?php if($row->published_by > 0) {
                            $userPublish = JFactory::getUser($row->published_by);
                             echo $userPublish->name;
                            } ?>
                        </a>
                          <?php else: ?>
                            <?php if($row->published_by > 0) {
                        ...
                        <td class="k2ui-center"><?php if($row->top_item == 0): ?> <span class="icon-unpublish" aria-hidden="true"></span> <?php else: ?> <span class="icon-publish" aria-hidden="true"></span> <?php endif; ?></td>
                        <?php if (K2_JVERSION != '30'): ?>
                        <td class="k2ui-order">
                            <?php if ($this->filter_featured == '1'): ?>
