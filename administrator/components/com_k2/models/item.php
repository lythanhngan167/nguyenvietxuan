...
class K2ModelItem extends K2Model
{
    ...
    public function save($front = false)
    {
        ...
        if ($row->id) {
            ...
            $current->load($row->id);
            if($row->top_item == 1 && $current->top_item==0){
                $row->top_item_date = K2_JVERSION == '15' ? $datenow->toMySQL() : $datenow->toSql();
            }
        } else {
        ...
        if (isset($_POST['send_notication']) && $_POST['send_notication'] == 1) {
            $datenow = JFactory::getDate();
            $userAdmin = JFactory::getUser();
            $query .= " status_notication = 1, ";
            $query .= " user_send_notification = ".$userAdmin->id.", ";
            $query .= " date_send_notification = '".$datenow."', ";
        }

        if($row->published == 1){
          $userAdmin = JFactory::getUser();
          $query .= " published_by = ".$userAdmin->id.", ";
        }
        ...
        if (isset($_POST['send_notication']) && $_POST['send_notication'] == 1 && $row->published == 1) {
            $category = JTable::getInstance('K2Category', 'Table');
            $category->load($row->catid);
            $aliascat = $category->alias;
            $itemURL = urldecode(JRoute::_(K2HelperRoute::getItemRoute($row->id.':'.urlencode($row->alias),$row->catid.':'.$aliascat)));
            $previewLink =  str_replace("/administrator/","",$itemURL);
            $previewLink =  str_replace("/bcavietnam2","",$previewLink);
            $web_url = JURI::root().$previewLink;
            $page_app = 'insurchannel://bizappco/NewsCommonPage/'.$row->catid;
            $row->introtext =  "[".$category->name."] - ".$row->introtext;

            $config = new JConfig();

            $sendAppAndWeb = 0;
            $str_parens = implode(', ', $config->categories_post_send_app_notification);
            $db = JFactory::getDbo();
            $query_categories = $db->getQuery(true);
            $query_categories->select('id');
            $query_categories->from($db->quoteName('#__k2_categories'));
            $query_categories->where($db->quoteName('published') . " = 1");
            $query_categories->where($db->quoteName('trash') . " = 0");
            $query_categories->where($db->quoteName('parent')." IN (".$str_parens.")");
            $db->setQuery($query_categories);
            $result = $db->loadObjectList();
            $result = array_column($result, 'id');
            if(in_array($row->catid,$result) || in_array( $row->catid,$config->categories_post_send_app_notification)){
                $sendAppAndWeb = 1;
            }

            $this->sendNotificationsPost($row->title,$row->introtext,$page_app,$web_url,$sendAppAndWeb);
        }
        ...
        switch (JRequest::getCmd('task')) {
            ...
            default:
                $msg = JText::_('K2_ITEM_SAVED');
                if ($front) {
                    $link = 'index.php?option=com_k2&view=itemlist&Itemid=491&task=user&id='.$user->id;
                } else {
	...
	public function sendNotificationsPost($title,$description,$page_app,$web_url,$sendAppAndWeb)
	{
			$title_notification = strip_tags($title);
			$content_notification = strip_tags($description);
			$page_app = $page_app;
      $web_url = $web_url;
			$tag_key = '';
			$tag_value = '';
      ...
      $this->sendMessageOnesignalNotification($title_notification,$content_notification,$tag_key,$tag_value,$segments,$page_app,$web_url,$sendAppAndWeb);

	}

	public function sendMessageOnesignalNotification($title,$content,$tag_key,$tag_value,$segments,$page_app,$web_url,$sendAppAndWeb){
      $content = array(
				"vi" => $content,
				"en" => $content
				);
			...
			// );
			$app_url = 'insurchannel://bizappco/NotifyPage/0';
      ...
      if($sendAppAndWeb == 1){
        $fields = array(
          ...
          'isAnyWeb' => true,
			...
			if ($_SERVER['HTTP_HOST'] == "bcavietnam.com" || $_REQUEST['test'] == 1) {
				$ch = curl_init();
