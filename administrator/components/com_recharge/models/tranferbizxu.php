...
class RechargeModelTranferbizxu extends JModelList
{
	...
	public function updateMoneyTranfer($user_id_from, $user_id_to, $money){
      ...
      if($change_money){
        ...
        $obj->title = 'Chuyển BizXu đến ' . $userSaleHistoryTo->name." - ".$userSaleHistoryTo->username. " - ".$userSaleHistoryTo->id_biznet;
        $obj->amount = $money;
	...
	public function updateMoneyReceive($user_id, $user_id_from, $money){
      ...
      if($change_money){
        ...
        $obj->title = 'Nhận BizXu từ '.$userSaleHistoryFrom->name." - ".$userSaleHistoryFrom->username. " - ".$userSaleHistoryFrom->id_biznet;
        $obj->amount = $money;
        $obj->created_date = date('Y-m-d H:i:s');
        $obj->type_transaction = 'tranfermoney';
        $obj->status = 'completed';
