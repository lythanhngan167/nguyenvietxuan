 ...
 * @since  1.6
 */
class RechargeModelExportrechargesbdm extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function getMoneySaleByMonth($saleid,$month,$year){
		...
		$query->select('sum(total) as total')
			->from('#__eshop_orders AS od')
			// ->where('od.created_by = '.$saleid)
			// ->where("od.create_date >= '".$first_month."'")
			//->where("od.create_date <= '".$end_month."'")
			->where("od.id > 0");
			// ->where('od.id = 3')
			// Join over the user groups table.
		$db->setQuery($query);
	...
	public function confirmPayment($userid, $monthyear, $total){
		...
		$payment->username = $this->getUserName($userid);
		$payment->user_id = $userid;
		$payment->month_year = $monthyear;
		$payment->total = $total;
		$payment->state = 1;
		$payment->created_by = $user->id;
		$payment->created_date = date("Y-m-d H:i:s");

		$result = JFactory::getDbo()->insertObject('#__check_payment', $payment);
		if($result){
			return 1;
		}else{
			return 0;
	...
	public function  checkPayment($userid, $monthyear){
		...
		// Join over the group mapping table.
		$query->select('count(*)')
			->from('#__check_payment AS cp')
			->where('cp.user_id = '.$userid)
			->where("cp.month_year = '".$monthyear."'");
			// ->where('od.id = 3')
			// Join over the user groups table.
		$db->setQuery($query);
		$result = $db->loadResult();
	...
	public function returnToStore($userid)
	{
		...
		$conditions = array(
		    $db->quoteName('sale_id') . ' = '.$userid,
		    $db->quoteName('status_id') . " != '7'");
	...
	public function getListUserSBDM() {
		$db    = $this->getDbo();
		$query = $db->getQuery(true);
		$query->select('u.id, u.name, u.username')
			->from('#__users AS u')
			->join('INNER','#__user_usergroup_map AS ug ON ug.user_id = u.id AND ug.group_id = 15')
			->where('u.block = 0');
		$db->setQuery($query);
		// echo $query->__toString();
		// die();
