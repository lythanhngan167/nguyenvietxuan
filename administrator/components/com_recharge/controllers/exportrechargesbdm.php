 ...
 * @since  1.6
 */
class RechargeControllerExportrechargesbdm extends JControllerAdmin
{
	/**
	...
	public function exportExcel(){
		...
		$list_recharge = $this->listRecharges($sbdm_user,$month,$year);
	...
	public function listRecharges($sbdm_user,$month,$year)
	{
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->select('a.id, a.code, created_by.name, created_by.username, a.amount, a.created_time, a.note, a.sbdm_id');
		...
		$query->join('INNER', '#__user_usergroup_map AS `u` ON `u`.user_id = a.`sbdm_id` AND u.group_id = 15');
		...
		if($sbdm_user != ''){
			$query->where($db->quoteName('a.sbdm_id') . '=' . $db->quote($sbdm_user));
		}
	...
	function exportAll($list_recharge)
	{
			...
			if (count($list_recharge) > 0) {
					...
					foreach ($list_recharge as $key => $recharge) {
								...
								$objPHPExcel->getActiveSheet()->SetCellValue('H' . $index, $recharge->note);
								$index = $index + 1;
					}
			}

			$objPHPExcel->getActiveSheet()->setTitle('Danh Sach Nap Tien BDM');
			// Save Excel 2007 file
			$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');

			$objWriter->save(JPATH_ADMINISTRATOR . DS . 'export' . DS . 'danhsachnaptienbdm.xlsx');
	}
