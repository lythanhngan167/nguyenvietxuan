 ...
 * @since  1.6
 */
class RechargeViewExportrecharge extends JViewLegacy
{
	/**
	 * The item data.
	 *
	...
	public function display($tpl = null){
		...
		if($_REQUEST['clear'] == 1){
			$this->state->set('filter.month', date('m'));
			$this->state->set('filter.year', date('Y'));
			$this->state->set('filter.day', date('d'));
			$this->state->set('filter.leader', '');
			$this->state->set('filter.search', '');
			$app =& JFactory::getApplication();
			$app->redirect('index.php?option=com_recharge&view=exportrecharge&filter[search]=');
		}
		...
		JToolBarHelper::title("Xuất Excel Nạp BizXu");
