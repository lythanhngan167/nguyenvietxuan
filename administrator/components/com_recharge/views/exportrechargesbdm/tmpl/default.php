...
$sbdm_user_selected =  $this->state->get('filter.sbdm', '');
...
<form action="<?php echo JRoute::_('index.php?option=com_recharge&view=exportrechargesbdm'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<?php endif; ?>
		...
		<!--Filter date start-->
		<select name="filter[sbdm]" id="sbdm" onchange="this.form.submit();">
			<option value="" <?php if($sbdm_user_selected == ''): ?> selected <?php endif; ?>>Tất cả BDM</option>
			<?php
				foreach($this->users as $user){
			?>
				<option <?php if($user->id == $sbdm_user_selected){ echo 'selected="selected"';} ?> value="<?php echo $user->id?>" ><?php echo  $user->name . " - ". $user->username ?></option>
			<?php
				}
			?>
		</select>
		...
		&nbsp;&nbsp;<a href="<?php echo JRoute::_('index.php?option=com_recharge&view=exportrechargesbdm&clear=1'); ?>"><button  type="button" class="btn btn-danger btn hasTooltip js-stools-btn-clear">Xóa</button></a>
...
function exportExcel(){
    ...
    jQuery.ajax({
        url: "<?php echo JUri::base(); ?>index.php?option=com_recharge&task=exportrechargesbdm.exportExcel",
        type : "POST",
          dataType:"text",
        data : {
             month : '<?php echo $this->state->get('filter.month', '') ?>',
             year: '<?php echo $this->state->get('filter.year', '') ?>',
             sbdmUser: '<?php echo $this->state->get('filter.sbdm', '') ?>'
        },
        success: function (result) {
            if (result == '1') {
                alert("Xuất Excel thành công, tải về ngay!");
                location.href = "<?php echo JUri::base(); ?>export/danhsachnaptienbdm.xlsx";
            }
            if (result == '0') {
