 ...
 * @since  1.6
 */
class RechargeViewExportrechargesbdm extends JViewLegacy
{
	/**
	 * The item data.
	 *
	...
	public function display($tpl = null){
		...
		$day =  $this->state->get('filter.sbdm', '');

		$this->users = $this->get('ListUserSBDM');
		...
		if($_REQUEST['clear'] == 1){
			$this->state->set('filter.month', date('m'));
			$this->state->set('filter.year', date('Y'));
			$this->state->set('filter.sbdm', '');
			$this->state->set('filter.leader', '');
			$this->state->set('filter.search', '');
			$app =& JFactory::getApplication();
			$app->redirect('index.php?option=com_recharge&view=exportrechargesbdm&filter[search]=');
		}
		...
		JToolBarHelper::title("Xuất Excel Nạp BizXu theo BDM");
