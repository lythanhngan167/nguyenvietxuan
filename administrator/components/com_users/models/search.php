 ...
 * @since  1.6
 */
class UsersModelSearch extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function getUserByPhoneOrBiznetID($phone_biznet_id){
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->select('a.*');
		...
		$query->where("(".$db->quoteName('a.username') . " = '".$phone_biznet_id."' OR ".$db->quoteName('a.id_biznet')." = '".$phone_biznet_id."' )");
	...
	public function checkUserBelongToBDM($user_id, $bdm_id){
		$oUser = $this->findParentUser($user_id,$bdm_id);
	}

	public function findParentUser($user_id,$bdm_id){
		if($user_id > 0){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('a.*');
			...
			$db->setQuery($query);
			$oUser = $db->loadObject();
			if($oUser->invited_id == $bdm_id){
				return 1;
			}else{
				if($oUser->invited_id > 0){
					$checkFind = $this->findParentUser($oUser->invited_id,$bdm_id);
					return $checkFind;
				}else{
					return 0;
				}
	...
	public function changeLevel($userid,$level)
	{
		...
		$sale = new stdClass();
		$userLevel   = $this->getUserByID($userid);
		$sale->id = $userid;
		$sale->level = $level;
		...
		if($result){
				...
				$log->created_by 	= $userAdmin->id;
				$log->modified_by 	= $userAdmin->id;
				$log->status		= 1;
				$log->user_id		= (int)$userid;
				$log->old_level	= $userLevel->level;
				$log->new_level	= $level;
				$log->state			= 1;
