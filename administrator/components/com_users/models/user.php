...
class UsersModelUser extends JModelAdmin
{
	...
	public function save($data)
	{
		...
		// Bind the data.
		if($data['groups'][0] == 2 || $data['groups'][0] == 3){
			$isMobile = $this->isMobile($data['username']);
			if(!$isMobile){
	 ...
	 * @since   1.6
	 */
	public function blockLanding(&$pks, $value = 1)
	{
		...
		foreach ($pks as $i => $pk)
		{
			...
			elseif ($table->load($pk))
			{
				...
				if ($allow)
				{
					...
					if ($table->block_landingpage == $value)
					{
						unset($pks[$i]);
						continue;
					}

					$table->block_landingpage = (int) $value;

					// If unblocking, also change password reset count to zero to unblock reset
					if ($table->block_landingpage === 0)
					{
						$table->resetCount = 0;
					}
