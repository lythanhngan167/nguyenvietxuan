 ...
 * @since  1.6
 */
class UsersModelReports extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function getUsersByLevelAndMoney($level,$money){
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->select('count(*)');
		$query->from($db->quoteName('#__users','a'));
		...
		$query->where($db->quoteName('a.money') . " >= ".$money);
		$query->where($db->quoteName('a.autobuy') . " = 1");
		$db->setQuery($query);
		$result = $db->loadResult();
		return $result;
	}

	public function getUsersByLevelAndAutobuy($level){
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->select('count(*)');
		$query->from($db->quoteName('#__users','a'));
	...
	public function getUsersByLevelAndAutobuyAndBuyTotal($level,$buy_total = 20,$money){
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->select('count(*)');
		$query->from($db->quoteName('#__users','a'));
		...
		$query->where($db->quoteName('a.buyall') . " > ".$buy_total);
		$db->setQuery($query);
		$result = $db->loadResult();
		return $result;
	}

	public function getUsersByLevelAndAutobuyAndBuyTotalLevel1($level,$buy_total = 20,$money){
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->select('count(*)');
		$query->from($db->quoteName('#__users','a'));
	...
	public function getUsersByLevelAndAutobuyAndBuyTotalFromTo($buy_total_from = 21,$buy_total_to = 25,$money){
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->select('count(*)');
		$query->from($db->quoteName('#__users','a'));
		...
		$query->where($db->quoteName('a.buyall') . " >= ".$buy_total_from);
		$query->where($db->quoteName('a.buyall') . " <= ".$buy_total_to);
		$db->setQuery($query);
		$result = $db->loadResult();
		return $result;
	}
	...
	public function countLandingpageChannel(){
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->select('count(*)');
		$query->from($db->quoteName('#__customers','a'));
		$query->where($db->quoteName('a.project_id') . " = 22");
		$db->setQuery($query);
		$result = $db->loadResult();
		return $result;
	}

	public function countMarketingChannel(){
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->select('count(*)');
		$query->from($db->quoteName('#__customers','a'));
