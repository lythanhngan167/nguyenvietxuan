...
class UsersModelUsers extends JModelList
{
	...
	public function getItems()
	{
		...
		if (empty($this->cache[$store]))
		{
			...
			foreach ($items as $item)
			{
				...
				$item->group_names = '';
				$item->note_count = 0;
				$user = JFactory::getUser($item->sbdm_id);
				$item->sbdm_name = $user->name;
				$item->sbdm_username = $user->username;
			}
	...
	protected function getListQuery()
	{
		...
		$groupId = $this->getState('filter.group_id');
		$groups  = $this->getState('filter.groups');
		$query->select('ct.country_name')->leftJoin('#__eshop_countries AS ct ON ct.id = a.province');
  		...
  		$db->setQuery($query);
			...
			if($result) {
				foreach($result as $row) {
					$len 	= strlen($row->username2);
					if(preg_match('/^[a-z][0-9A-Z]{8,}+$/i', $row->username2)) {
						$bric 	= substr($row->username2, $len - 8, 8);
						if(preg_match('/^[0-9]{8,}$/i', $bric)) {
							$object = new stdClass();
							$object->id = $row->id;
							$object->bric_code = $bric;
							$updateResult = JFactory::getDbo()->updateObject('#__users', $object, 'id');
							if(!$result) {
								echo 'Lỗi copy' . $object;
								die();
							}
						}
					}
    ...
    $db = JFactory::getDbo();
    $sql = 'SELECT a.*, (SELECT COUNT(*) FROM `#__customers` WHERE `sale_id` = 0 AND `state` = 1 AND `project_id` = a.id AND category_id = 151 ) as remain_customer FROM `#__projects` as a WHERE a.id = '.(int)$id;
