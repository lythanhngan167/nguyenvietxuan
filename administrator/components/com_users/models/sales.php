 ...
 * @since  1.6
 */
class UsersModelSales extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		if ($groupId || isset($groups))
		{
			...
			$groupId = 3;
			if($_GET['name'] !=''){
				$query->where("(a.username LIKE '".$_GET['name']."' OR a.name LIKE '".$_GET['name']."')");
			}
			if($_GET['leader'] > 0){
				$query->where("a.parent_id = ".$_GET['leader']);
			}
