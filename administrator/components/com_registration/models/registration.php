 ...
 * @since  1.6
 */
class RegistrationModelRegistration extends \Joomla\CMS\MVC\Model\AdminModel
{
	/**
	 * @var      string    The prefix to use with controller messages.
	 * @since    1.6
	 */
	protected $text_prefix = 'COM_REGISTRATION';

	/**
	 * @var   	string  	Alias to manage history control
	 * @since   3.2
	 */
	public $typeAlias = 'com_registration.registration';
	...
	public function getForm($data = array(), $loadData = true)
	{
            ...
            // Get the form.
            $form = $this->loadForm(
                    'com_registration.registration', 'registration',
                    array('control' => 'jform',
                            'load_data' => $loadData
                    )
            );
	...
	/**
	 * Method to duplicate an Registration
	 *
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_registration'))
		{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = Factory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__registration');
				$max             = $db->loadResult();
