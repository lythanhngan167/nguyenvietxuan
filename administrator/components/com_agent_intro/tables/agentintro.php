...
/**
 * agentintro Table class
 *
 * @since  1.6
 */
class Agent_introTableagentintro extends \Joomla\CMS\Table\Table
{
	...
	{
		JObserverMapper::addObserverClassToClass('JTableObserverContenthistory', 'Agent_introTableagentintro', array('typeAlias' => 'com_agent_intro.agentintro'));
		parent::__construct('#__agent_intro', 'id', $db);
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!Factory::getUser()->authorise('core.admin', 'com_agent_intro.agentintro.' . $array['id']))
		{
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_agent_intro/access.xml',
				"/access/section[@name='agentintro']/"
			);
			$default_actions = Access::getAssetRules('com_agent_intro.agentintro.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	public function check()
	{
		...
		if ($files['image'][0]['size'] > 0)
		{
			...
			$oldFiles = Agent_introHelper::getFiles($this->id, $this->_tbl, 'image');
	...
	protected function _getAssetName()
	{
		...
		return 'com_agent_intro.agentintro.' . (int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		$assetParent->loadByName('com_agent_intro');
	...
	public function delete($pk = null)
	{
		...
		if ($result)
		{
			...
			{
			case 'string':
				JFile::delete(JPATH_ROOT . '/images/landingpage/' . $this->image);
			break;
			default:
			foreach ($this->image as $imageFile)
			{
				JFile::delete(JPATH_ROOT . '/images/landingpage/' . $imageFile);
			}
			}
		}
