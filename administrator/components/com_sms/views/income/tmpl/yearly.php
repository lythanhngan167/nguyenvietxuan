...
<form action="<?php echo JRoute::_('index.php?option=com_sms&view=income');?>" method="post" name="adminForm" id="adminForm">
	...
	<?php if (empty($this->items)) : ?>
		...
		<?php else : ?>
			...
			$header .= '<p> '.JText::_('LABEL_INCOME_YEAR_LIST').'</p>';
			$header .= $header_con;
			$header .= '</div>';
			echo $header;
			?>
		...
		<thead>
			<tr>
			  <th  class=" center">#</th>
				<th  class=" center"><?php echo JText::_('LABEL_INCOME_YEAR'); ?></th>
				<th style="text-align: right;padding-right: 100px;" ><?php echo JText::_('LABEL_INCOME_AMMOUNT'); ?></th>
			</tr>
		</thead>
		<tbody>
		  <?php 
		  ...
		  ?>
			...
			<tr class="row<?php echo $i % 2; ?>">
			  <td class="center"></td>
				<td class="center"></td>
				<td style="text-align: right;color: green;padding-right: 100px;"><?php echo JText::_('LABEL_INCOME_TOTAL'); ?>: <?php echo SmsHelper::getCurrency($total); ?></td>
			</tr>
		</tbody>
	</table>
	</div>
