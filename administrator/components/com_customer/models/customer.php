 ...
 * @since  1.6
 */
class CustomerModelCustomer extends JModelAdmin
{
	/**
	 ...
	 * @since   3.2
	 */
	public $typeAlias = 'com_customer.customer';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm(
			'com_customer.customer', 'customer',
			array('control' => 'jform',
				'load_data' => $loadData
			)
		);
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_customer'))
		{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__customers');
				$max             = $db->loadResult();
