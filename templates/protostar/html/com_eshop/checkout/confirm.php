...
$location = $session->get('shipping_location', 50);
$cartAmount = 0;
        ...
        */
        ...
        <table class="table table-hover">
            ...
            foreach ($this->cartData as $key => $product) {
                ...
                if ($location != 50 && SUtil::checkLimitArea($product['product_id'])) {
                    $limitShipping = true;
                    $limitText = 'Sản phẩm tạm thời chưa giao hàng tới khu vực của bạn';
                }
                ?>
                <tr>
                    <td data-content="<?php echo JText::_('ESHOP_PRODUCT_NAME'); ?>">
                        ...
                        <?php if ($limitText): ?>
                            <div class="limit_shipping_area"><?= $limitText ?></div>
                        <?php endif; ?>
                    </td>
            ...
            foreach ($this->totalData as $data) {
                if($data['name'] == 'sub_total'){
                    $cartAmount = $data['value'];
                }
                ?>
                <tr>
...
<?php
$config = new Sconfig();
if ($this->total > 0 && $cartAmount >= $config->minCartAmount) {

    if ($this->paymentClass->getName() != 'os_squareup' && ! $limitShipping) {
        ?>
   ...
   ?>
   <div class="checkout-term" style="color: orange; padding-top:15px;">
        <?php echo $config->shipping_fee; ?>
   </div>

   <?php
} else {
    ?>
    <div class="message" style="color: red">Đặt hàng online chỉ áp dụng cho đơn hàng tối
        thiểu <?= number_format($config->minCartAmount) ?> đ
    </div>
    <!--<button type="submit" class="btn btn-outline-success" id="button-confirm" ><?php /*echo JText::_('ESHOP_CONFIRM_ORDER'); */
    ?></button>-->
